...
function init()
  ...
  refresh()
  skillsWindow:setup()
  ProtocolGame.registerExtendedOpcode(104, setSkillsCustom)
end
...
function terminate()
  ...
  ProtocolGame.unregisterExtendedOpcode(104, setSkillsCustom)
end

function setSkillsCustom(protocol, opcode, buffer)
  local skill, level, percent = string.match(buffer, "|(.-)|(%d+)|(%d+)|")
  setSkillValue(skill, level)
  setSkillPercent(skill, 100 - percent, tr('You have %s percent to go', percent))
end
...
function refresh()
  ...
  skillsWindow:setContentMaximumHeight(290 + 50)
end
