...
function init()
  ...
  connect(g_game, { onGameEnd = offline, onGameStart = online })
  ...
  vocationLabel = healthInfoWindow:recursiveGetChildById('vocation')
...
function getVocationIconPath( vocation )

  if vocation > 4 and vocation < 9 then
    vocation = vocation - 4
  end

  local elements = { [1] = "air" , [2] = "earth", [3] = "water", [4] = "fire", [9] = "avatar"}
  local path = "/images/game/elements/characterlist/"

  return path .. elements[vocation]
end

function onVocationChange(vocation, oldVocation)
  local player = g_game.getLocalPlayer()
    vocationLabel:setImageSource(getVocationIconPath(player:getVocation()))
  end
...
function checkHealthAlert()
  ...
  if player  and map then
    local percent = player:getHealth()/player:getMaxHealth()
    local current_shader = map:getMapShader():getProgramId()
     local defaul_shader = g_shaders.getDefaultMapShader():getProgramId()
     local red_alert_shader = g_shaders.getShader("red_alert"):getProgramId()
     if (current_shader ~= defaul_shader and current_shader ~= red_alert_shader)then
     return 
     end
    if percent < 0.25 then
     map:setMapShader(g_shaders.getShader("red_alert"));
    else
