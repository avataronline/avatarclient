...
function init()    
    ...
    tutorialButton = modules.client_topmenu.addLeftGameButton('tutorialButton', tr('Tutorial'), 'imgs/tutorial', onClickTutorial)
    ...
    ProtocolGame.registerExtendedOpcode(ExtendedIds.GameTutorial, parseGameTutorial)
end
...
function terminate()
    ...
    ProtocolGame.unregisterExtendedOpcode(ExtendedIds.GameTutorial, parseGameTutorial)
--g_game.unhandleExtended(loginCountExtended)
end

function parseGameTutorial(protocol, opcode, buffer)
    local msg = InputMessage.create()
    msg:setBuffer(buffer)

    local packet = msg:getStrTable()

    if packet.action == "show" then
        onOpenTutorial()
    elseif packet.action == "hide" then
        hideAll()
    end
end
